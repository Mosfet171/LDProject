...
int main() {
	...
	for( i = 0; i < N_THREADS; i++ ) {
		...
		rc = pthread_create(&threads[i], NULL, &exec_on_thread, (void*)i);
	...
	*/
	...
	thread th0 (exec_on_thread, 0);
	thread th1 (exec_on_thread, 1);
	thread th2 (exec_on_thread, 2);
	thread th3 (exec_on_thread, 3);
	thread th4 (exec_on_thread, 4);
	thread th5 (exec_on_thread, 5);
	thread th6 (exec_on_thread, 6);
	thread th7 (exec_on_thread, 7);
